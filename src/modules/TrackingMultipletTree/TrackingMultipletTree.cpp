/**
 * @file
 * @brief Implementation of module TrackingMultipletTree
 *
...
#include "TrackingMultipletTree.h"
...
TrackingMultipletTree::TrackingMultipletTree(Configuration& config, std::vector<std::shared_ptr<Detector>> detectors)
...
void TrackingMultipletTree::initialize() {
    ...
    matchingDistanceAtScattererX = new TH1F("matchingDistanceAtScattererX", title.c_str(), 500, -10., 10.);
    title = "Matching distance Y at scatterer;distance y [mm];multiplet candidates";
    matchingDistanceAtScattererY = new TH1F("matchingDistanceAtScattererY", title.c_str(), 500, -10., 10.);

    title = "Multiplet offset X at scatterer;offset x [um];multiplets";
    multipletOffsetAtScattererX = new TH1F("multipletOffsetAtScattererX", title.c_str(), 500, -300., 300.);
    ...
    TTree *tree = new TTree("multiplet_data", "Data from Multiplet Analysis");
...
double TrackingMultipletTree::calculate_average_timestamp(const Track* track) {
...
bool TrackingMultipletTree::duplicated_hit(const Track* a, const Track* b) {
...
template <class T> T TrackingMultipletTree::remove_duplicate(T tracks) {
...
TrackVector TrackingMultipletTree::refit(MultipletVector multiplets) {
...
TrackVector TrackingMultipletTree::find_multiplet_tracklets(const streams& stream,
...
void TrackingMultipletTree::fill_tracklet_histograms(const streams& stream, const selection& selected, TrackVector tracklets) {
...
StatusCode TrackingMultipletTree::run(const std::shared_ptr<Clipboard>& clipboard) {
    ...
    for(auto& multiplet : multiplets) {
        ...
        multipletKinkAtScattererXvsGlobalXY->Fill(pos_x, pos_y, kinkX*kinkX);
        ...
        Double_t posx, posy, kinkx, kinky;
