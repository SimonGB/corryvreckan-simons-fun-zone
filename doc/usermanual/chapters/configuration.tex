...
, and passing their values using the \command{position_parameters} keyword, using explicit units. This feature is not supported together with detector alignment modules. The following procedure is suggested to perform time-dependent alignment. First, run the alignment modules on a small number of events, such that statistics is sufficient for alignment, but the relative drifts are negligable. Second, process the full run without using alignment modules. Thirs, check \command{residualsXVsEventTime} and \command{residualsYVsEventTime} and fit an arbitrary function describing the profile, choosing an appropiate binning (so as not to artificially improove the residual width). The last step the fitted functios are passed to corry, as discussed above, and the run is re-processed.
